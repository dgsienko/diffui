...
def _build_agent_context() -> tuple[str, str, str, int] | tuple[None, str, str, int]:
    ...
    prompt = (
        ...
        f"IMPORTANT: treat these comments as a conversation:\n"
        ...
        f"- When you are confident a comment has been fully addressed, set its 'status' field to "
        f"'resolved' in the JSON. Do NOT remove the comment object; only change its status.\n"
        f"- If a comment is unclear or you're unsure how to address it, reply with a question "
        f"and LEAVE the status as 'open'. Do not guess.\n"
